...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONFIG = {
        whatsappNumber: '554195692590',
        mapCenter: [-25.4087, -49.1877],  // Pinhais - PR
        mapZoom: 15
    };
    ...
    const initScrollAnimations = () => {
        ...
        if (document.getElementById('home').classList.contains('active')) {
            ...
            // Revela Cabeçalho e Cards dos Diferenciais via Timeline
            const diffTimeline = gsap.timeline({
                scrollTrigger: {
            ...
            diffTimeline.from('.differentials-section .gs-reveal-header', {
                opacity: 0,
                y: 40,
                duration: 0.6,
                ease: 'power3.out',
            ...
            .from('.differentials-section .gs-reveal-card', {
                opacity: 0,
                y: 50,
                duration: 0.8,
                stagger: 0.12,
        ...
        // Seção Caminhões ativa
        if (document.getElementById('caminhoes') && document.getElementById('caminhoes').classList.contains('active')) {
            gsap.from('#caminhoes .gs-reveal-header', {
                opacity: 0,
                y: 40,
                duration: 0.8,
                ease: 'power3.out'
            ...
            gsap.from('#caminhoes .gs-reveal-left', {
                opacity: 0,
                x: -50,
                duration: 0.8,
                ease: 'power2.out',
                scrollTrigger: {
                    trigger: '.fleet-specs-grid',
                    start: 'top 75%'
                }
            });
            
            gsap.from('#caminhoes .gs-reveal-right', {
                opacity: 0,
                x: 50,
                duration: 0.8,
                ease: 'power2.out',
        ...
        // Seção Sobre ativa
        if (document.getElementById('sobre') && document.getElementById('sobre').classList.contains('active')) {
            gsap.from('#sobre .gs-reveal-header', {
                opacity: 0,
                y: 40,
                duration: 0.8,
                ease: 'power3.out',
        ...
        // Delay aumentado para garantir que o layout esteja totalmente calculado
    ...
    function initGalleryFilters() {
        ...
        // Garante que o primeiro botão começa com classe active
        filterBtns.forEach(b => b.classList.remove('active'));
        filterBtns[0].classList.add('active');

        // Lazy loading: fade-in ao carregar cada imagem
        const lazyImgs = galleryGrid.querySelectorAll('.gallery-img[loading="lazy"]');
        ...
        filterBtns.forEach(btn => {
            ...
            btn.addEventListener('click', () => {
                ...
                // Animate filter transition with GSAP if available
                if (isGsapActive) {
                    gsap.to(items, {
                        opacity: 0,
                        scale: 0.92,
                        ...
                        onComplete: () => {
                            ...
                            // Fade back in only visible ones
                            ...
                            // Refresh scroll triggers to avoid whitespace/bugs
    ...
    function router() {
        ...
        if (progressBar) {
            ...
            gsap.to(progressBar, { width: '70%', duration: 0.4, ease: 'power2.out' });
        }
        ...
        const transitionTimeline = gsap.timeline({
            ...
            onComplete: () => {
                ...
                setup3DTiltCards();
                initLightbox();
                // Completa e some a barra de progresso
                if (progressBar) {
                    gsap.to(progressBar, {
                        width: '100%',
                        duration: 0.25,
                        ease: 'power2.out',
                        onComplete: () => {
                            gsap.to(progressBar, { opacity: 0, duration: 0.4, delay: 0.1, onComplete: () => gsap.set(progressBar, { width: '0%' }) });
                        }
                    });
                }
            }
    ...
    function triggerWhatsAppRedirect() {
        ...
        let text = `*Nova Solicitação de Orçamento - CVC Locações*\n\n`;
        text += `*Nome:* ${name}\n`;
        ...
        if (!mapInitialized) {
            ...
            marker.bindPopup(`
                <div style="font-family: 'Inter', sans-serif; padding: 8px 4px;">
                    <h5 style="color: #FF5E00; font-family: 'Outfit', sans-serif; margin: 0 0 6px 0; font-size: 1rem; font-weight: 700;">CVC Locações</h5>
                    <p style="margin: 0; font-size: 0.82rem; color: #444; line-height: 1.5;">
                        Rua Célio José Pivova, 129<br>
                        Pinhais – PR
                    </p>
                </div>
            `, {
    ...
    function initStaticRouter() {
        ...
        // Listener para preencher formulário ao clicar em solicitar equipamento
        document.addEventListener('click', (e) => {
            const prefillTarget = e.target.closest('[data-prefill]');
            if (prefillTarget) {
                const prefillValue = prefillTarget.getAttribute('data-prefill');
                const selectEl = document.getElementById('service_type');
                if (selectEl) {
                    if (prefillValue === 'VW 19.390') {
                        selectEl.value = 'Caminhão Munck VW 19.390';
                    }
                }
            }
        });
